...
import { ICommunityRepository } from "@/lib/types/community.type";
...
export class ReportService implements IReportService {
  ...
  async createReport(
    ...
    if (!data.reportedEntityId || !data.reportedEntityType || !data.reason) {
      throw new Error(
        "Validation Error: Missing required fields (reportedEntityId, reportedEntityType, reason)."
      );
    }
    ...
    if (!validTypes.includes(data.reportedEntityType)) {
      throw new Error(
        "Validation Error: Invalid reported entity type. Must be 'POST' or 'COMMENT'."
      );
    }

    const entityExists = await this.reportRepository.entityExists(
      data.reportedEntityId,
      data.reportedEntityType
    );
    if (!entityExists) {
  ...
  async handleReportDecision(
    ...
    // Validate admin ownership - admin must own the community the reported entity belongs to
    if (this.communityRepository) {
      const communityId = await this.reportRepository.getCommunityIdForReport(
        report
      );
      if (communityId) {
        ...
        if (!isOwner) {
          throw new Error(
            "Unauthorized: You can only handle reports in communities you own"
          );
        }
      }
    }
    ...
    const newStatus: "RESOLVED" | "REJECTED" =
      decision === "APPROVE" ? "RESOLVED" : "REJECTED";
    ...
    if (decision === "APPROVE") {
      try {
        ...
        throw new Error(
          `Report approved but failed to delete entity: ${
